...
#if defined(NDEBUG)
# error "Rapture Core cannot be compiled without assertions."
#endif
                              ...
                              std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s RAP additional fees, %d delta bytes\n",
...
/*
NOTE:   unlike rapture we are using PREVIOUS block height here,
...
{
    double dDiff;
    CAmount nSubsidyBase = 14; //added =14
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    if(nHeight > 29000) ret = blockValue / 1.27;
    return ret;
}
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // RAPTURE : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(RAPTURE): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(RAPTURE): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // RAPTURE : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(RAPTURE): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(RAPTURE): spork is off, skipping transaction locking checks\n");
    }
