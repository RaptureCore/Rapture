...
*
*   Why is this needed?
*   - In Rapture some blocks are superblocks, which output much higher amounts of coins
...
bool IsBlockPayeeValid(const CTransaction& txNew, int nBlockHeight, CAmount blockReward)
{
    ...
    if(sporkManager.IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT) && !mnpayments.IsTransactionValid(txNew, nBlockHeight)) {
		LogPrintf("IsBlockPayeeValid -- ERROR: Invalid rapture masternode payment detected at height %d: %s", nBlockHeight, txNew.ToString());
        return false;
    } else if (mnpayments.IsTransactionValid(txNew, nBlockHeight)){
		LogPrintf("IsBlockPayeeValid -- Valid rapture masternode payment detected at height %d: %s", nBlockHeight, txNew.ToString());
	}
...
{
    if(fLiteMode) return; // disable all Rapture specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f RAPTURE\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
