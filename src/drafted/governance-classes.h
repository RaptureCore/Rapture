 ...
 *
 *   // network
 *   CRaptureNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.rapture.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CRaptureNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CRaptureNetwork(UniValue objIn)
    {
...
// // can be under: RaptureNetwork
// //   -- signature requirements : Key1(User)
// class CRaptureNetworkVariable : public CGovernanceObject
// {
// private:
