...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const Consensus::Params& params) {
    /* current difficulty formula, rapture - DarkGravity v3, written by Evan Duffield - evan@rapture.org */
...
unsigned int GetNextWorkRequiredRAP(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    if ((pindexLast->nHeight+1) % params.DifficultyAdjustmentInterval() != 0)
    {
        if (params.fPowAllowMinDifficultyBlocks)
        {
            ...
            // If the new block's timestamp is more than 2* 70 seconds
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    if (pindexLast->nHeight + 1 >= params.nPowDGWHeight) {
        ...
        return GetNextWorkRequiredRAP(pindexLast, pblock, params);
    }
}
