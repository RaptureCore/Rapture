...
const char * const BITCOIN_CONF_FILENAME = "rapture.conf";
const char * const BITCOIN_PID_FILENAME = "raptured.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "rapture" is a composite category enabling all Rapture-related debug output
            if(ptrCategory->count(string("rapture"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RaptureCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RaptureCore
    // Mac: ~/Library/Application Support/RaptureCore
    // Unix: ~/.rapturecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RaptureCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RaptureCore";
#else
    // Unix
    return pathRet / ".rapturecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty rapture.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override rapture.conf
