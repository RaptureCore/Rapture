...
{
    // TODO: This is different from what we have in Rapture which only calls ConnectNode from OpenNetworkConnection
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Rapture users should be well used to small numbers with many decimal places by now :)
...
void CConnman::AcceptConnection(const ListenSocket& hListenSocket) {
    ...
    if(fMasterNode && !masternodeSync.IsSynced()) {
       ...
       // CloseSocket(hSocket);  //allow connection even if mn isn't synced
        return;
    }
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Rapture Core " + FormatFullVersion();
...
bool CConnman::BindListenPort(const CService &addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Rapture Core is probably already running."), addrBind.ToString());
        else
